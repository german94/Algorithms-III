...
void mostrar_resultado (vector<std::pair <vector<int>, int> >a, int cantidad_total);
...
cout<<"Soluciones:"<<endl;
	...
	for(int i = 0; i <entradas.size(); i++)
	   	{	
	   		int** pToFree = entradas[i].first;
	   		for(int k =0; k<entradas[i].second.first; k++)
	   			delete[] pToFree[k];
	   		delete[] pToFree;
		}
    return 0;
...
void mostrar_resultado (vector<std::pair <vector<int>, int> > camiones, int cantidad_total )
{// P camionR camion mostr camionr_result camiondo EL RESULT camionDO
	int i =0;
	int cantidad = camiones.size();
	cout << cantidad << " ";
	for (int i = 1; i <= cantidad_total ; i++)
	{
		for (int a = 0; a< camiones.size(); a++) 
		{
		for(int j = 0; j < camiones[a].first.size(); j++)
		{
			if ( camiones[a].first[j] == i) {
				cout <<  a+1 << " " ;
				}
		}
 }
...
{ //ES LA FUNCION PRINCIPAL LA CUAL LLAMA A ubicar_siguientes_productos , ESTA ES LA FUNCION QUE VA A PERMITIR 
	...
	while(!termine) 
	{
		...
		clock_t end = clock();
			...
			mostrar_resultado(camiones, cantidad_total);
