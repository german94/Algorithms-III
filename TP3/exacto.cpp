...
{ 
	...
	for (int i = 0; i < vertices; i++){
		for (int j = i+1; j < vertices; j++){
			suma_solucion = suma_solucion + pesos[i][j];
		} // INICIALIZO A SUMA__SOLUCION COMO LA SUMA DE TODOS LOS CONJUNTOS
	}
	if (suma_solucion == 0) // SI ES IGUAL A CERO, DE ENTRADA ENTONCES TODOS LOS ELEMENTOS
		//ENTRAN EN UN SOLO CONJUNTO Y FIN
	{
		vector<int> solo_un_conjunto;
		for(int i = 0; i < vertices; i++){
			solo_un_conjunto.push_back(i+1); //meto todos los vertices en un solo conjunto
		}
		solucion.push_back(solo_un_conjunto);
	}
	else{
	//poda(suma_solucion, pesos, vertices, particiones, solucion);// PARA USAR LA PODA HAY QUE COMENTAR 
		//DESDE 143 - 157 Y 170. QUE ANULAN EMPEZAR CON LA SUMA TOTAL
		for (int conjuntos_disponibles = 0; conjuntos_disponibles < particiones; conjuntos_disponibles++ ){	
